...
for day_ago in day_ago_list : 
    target_date = (date.today() - timedelta(days=day_ago)).strftime("%Y%m%d")
    ...
    pd.DataFrame(movie_list).to_csv(f'../data/raw/movie_info_api/movie_info_api_{target_date}.csv', index=False, encoding='utf-8-sig')